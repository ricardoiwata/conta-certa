...
function generateHTMLReport(
  ...
  const linhasReceitas = receitas
    .map(
      ...
      <tr>
        ...
        <td>${r.descricao}</td>
        <td><span class="category-badge success">${
          r.origem || "Sem origem"
        }</span></td>
        <td style="text-align: right; color: ${
          COLORS.primary_600
        }; font-weight: 600;">R$ ${r.valor.toFixed(2)}</td>
      </tr>
    `
    )
    .join("");
  ...
  const linhasCategoriasD = Object.entries(categoriasD)
    .map(
      ...
      <tr>
        <td>${cat}</td>
        <td style="text-align: right; font-weight: 700;">R$ ${val.toFixed(
          2
        )}</td>
      </tr>
    `
  ...
  return `
    ...
    <head>
      ...
      <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { max-width: 100%; margin: 0; background: ${
          COLORS.white
        }; overflow: hidden; }
    ...
    COLORS.primary_600
  } 100%); padding: 40px 30px; text-align: center; color: ${
    COLORS.white
        ...
        .summary-card.receita { background: linear-gradient(135deg, ${
          COLORS.primary_300
        }20 0%, ${COLORS.primary_200}20 100%); border: 2px solid ${
    COLORS.primary_300
  }; }
        .summary-card.despesa { background: linear-gradient(135deg, ${
          COLORS.error_500
        }20 0%, ${COLORS.error_500}30 100%); border: 2px solid ${
    COLORS.error_500
  }; }
        .summary-card.saldo { background: linear-gradient(135deg, rgba(100, 150, 100, 0.1) 0%, rgba(70, 120, 70, 0.1) 100%); border: 2px solid ${
          COLORS.primary_600
        }; }
        .summary-card label { display: block; font-size: 11px; color: ${
          COLORS.n_400
        }; margin-bottom: 8px; font-weight: 600; }
        .summary-card .value { font-size: 22px; font-weight: 700; }
        .summary-card.receita .value { color: ${COLORS.primary_600}; }
        .summary-card.despesa .value { color: ${COLORS.error_500}; }
        .summary-card.saldo .value { color: ${
          saldo >= 0 ? COLORS.primary_600 : COLORS.error_500
        }; }
        .section { margin-bottom: 15px; page-break-inside: avoid; }
        .section-title { background: linear-gradient(135deg, ${
          COLORS.primary_700
        } 0%, ${COLORS.primary_600} 100%); color: ${
    COLORS.white
  }; padding: 12px 16px; border-radius: 8px; font-size: 14px; font-weight: 700; margin-bottom: 16px; display: flex; align-items: center; gap: 8px; }
        ...
        .graphic-container h3 { font-size: 13px; margin-bottom: 12px; color: ${
          COLORS.n_900
        }; font-weight: 600; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 15px; font-size: 12px; }
        th { background: ${COLORS.n_050}; color: ${
    COLORS.n_900
  }; padding: 10px; text-align: left; font-weight: 700; border-bottom: 2px solid ${
    COLORS.primary_300
  }; }
        td { padding: 9px 10px; border-bottom: 1px solid ${
          COLORS.n_150
        }; color: ${COLORS.n_900}; }
        .total-row { background: ${
          COLORS.n_050
        }; font-weight: 700; border-top: 2px solid ${COLORS.primary_300}; }
        .category-badge { display: inline-block; background: ${
          COLORS.primary_050
        }; color: ${
    COLORS.primary_700
  }; padding: 3px 8px; border-radius: 12px; font-size: 11px; font-weight: 600; border: 1px solid ${
    COLORS.primary_300
  }; }
        .category-badge.success { background: ${COLORS.primary_050}; color: ${
    COLORS.primary_600
  }; border-color: ${COLORS.primary_300}; }
        .footer { margin-top: 15px; padding-top: 15px; border-top: 1px solid ${
          COLORS.n_200
        }; text-align: center; color: ${
    COLORS.n_400
  }; font-size: 10px; page-break-before: avoid; }
        ...
        <div class="content">
          <div class="summary-grid">
            ...
            <div class="summary-card saldo">
              <label>📊 Saldo</label>
              <div class="value">${saldo >= 0 ? "+" : ""}R$ ${saldo.toFixed(
    2
  )}</div>
            </div>
          </div>
              ...
              <table>
                ...
                <tbody>
                  ...
                  <tr class="total-row">
                    ...
                    <td style="text-align: right;">R$ ${totalDespesas.toFixed(
                      2
                    )}</td>
                  </tr>
                </tbody>
...
function gerarGraficoCategoriasD(categorias: Record<string, number>): string {
  ...
  // Paleta de cores diversificada para melhor distinção entre categorias
  const cores = [
    "#3B82F6", // Blue 500
    "#EF4444", // Red 500
    "#F59E0B", // Amber 500
  ...
  items.forEach(([cat, valor], index) => {
    const cor = cores[index % cores.length];
    const itemId = `legend-item-${uniqueId}-${index}-${Math.random()
      .toString(36)
      .substr(2, 5)}`;
    svg += `<div id="${itemId}" style="margin: 4px 0;"><span style="display: inline-block; width: 10px; height: 10px; background: ${cor}; border-radius: 2px; margin-right: 6px;"></span><span style="font-size: 10px;">${cat}: R$ ${valor.toFixed(
      2
    )}</span></div>`;
  });
  svg += "</div>";
...
function agruparPorOrigem(
  items: { origem?: string; valor: number }[]
): Record<string, number> {
  return items.reduce((acc, item) => {
...
function formatarData(dataStr: any): string {
  ...
  try {
    ...
    if (typeof dataStr === "object" && dataStr !== null) {
      ...
      (dataStr.includes("T") ||
        (dataStr.length === 10 && dataStr.includes("-")))
    ) {
